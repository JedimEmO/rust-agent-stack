...
async fn test_otel_usage_tracker() {
    ...
    let user = AuthenticatedUser {
        ...
        permissions: vec!["read".to_string(), "write".to_string()].into_iter().collect(),
        metadata: None,
    };
    ...
    let context = RequestContext::jsonrpc("getStatus".to_string())
...
async fn test_otel_method_duration_tracker() {
    ...
    // Test with authenticated user
    tracker.track_duration(&context, Some(&user), Duration::from_millis(250)).await;
    
    // Test with anonymous user
    tracker.track_duration(&context, None, Duration::from_millis(100)).await;
}
...
async fn test_standard_setup() {
    let setup = standard_setup("standard_service")
        .expect("Failed to create standard setup");
...
async fn test_various_duration_scales() {
    ...
    tracker.track_duration(&context, None, Duration::from_nanos(100)).await;
    tracker.track_duration(&context, None, Duration::from_micros(500)).await;
    tracker.track_duration(&context, None, Duration::from_millis(50)).await;
    tracker.track_duration(&context, None, Duration::from_secs(2)).await;
}
